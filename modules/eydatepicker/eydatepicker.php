...
(7, 7, 0, '08:00-12:00,13:00-17:00');
    ...
    function hookDisplayAdminOrder($params) {
        ...
        if (!empty($info['shipping_date'])) {			
			...
			$this->smarty->assign('basedir', __PS_BASE_URI__);
    ...
    function hookProcessCarrier($params) {
		...
		$this->context->cookie->shipping_date=date($this->context->language->date_format_lite, strtotime(Tools::getValue('shipping_date')));				
		//$this->context->cookie->shipping_date=Tools::getValue('shipping_date');		
		$this->context->cookie->shipping_hour=Tools::getValue('shipping_hour');
