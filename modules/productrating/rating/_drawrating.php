...
if ($static == 'static') {
		...
		$static_rater[] .= '</ul>';
		$static_rater[] .= '<p class="static">'.$productrating->traduz( 'Rating' ).': <strong> '.$rating1.'</strong>/'.$units.' ('.$count.' '.$tense.' '.$productrating->traduz( 'cast' ).') </p>';
		$static_rater[] .= '</div>';
		$static_rater[] .= '</div>'."\n\n";
      ...
      for ($ncount = 1; $ncount <= $units; $ncount++) { // loop from 1 to the number of units
           if(!$voted) { // if the user hasn't yet voted, draw the voting stars
              $rater.='<li><a href="../modules/productrating/rating/db.php?j='.$ncount.'&amp;q='.$id.'&amp;t='.$ip.'&amp;c='.$units.'" title="'.$productrating->traduz('Give your rating now').': '.$ncount.' '.$productrating->traduz('out of').' '.$units.'" class="r'.$ncount.'-unit rater" rel="nofollow">'.$ncount.'</a></li>';
           }
      }
