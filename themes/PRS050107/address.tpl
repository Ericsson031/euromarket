...
{literal}
	$(document).ready(function() {		
		$("#add_adress").submit(function(){
			$('#phone').val($('#phone_prefix').val()+$('#phone').val());			
			$('#phone_mobile').val($('#phone_mobile_prefix').val()+$('#phone_mobile').val());
			});
...
<form action="{$link->getPageLink('address', true)}" method="post" class="std" id="add_adress">
	...
	{foreach from=$ordered_adr_fields item=field_name}
		...
		<p class="required text">
			<label for="address1">{l s='Building'}<!--Address1--> <sup>*</sup></label>
		...
		<p class="required text">
			<label for="address2">{l s='Flat number'} <!--Address2--><sup>*</sup></label>
		...
		<p class="required text" style="display:none;">
			...
			<input type="text" name="city" id="city" value="Dubai{if isset($smarty.post.city)}{$smarty.post.city}{else}{if isset($address->city)}{$address->city}{/if}{/if}" maxlength="64" />
		</p>
		{*
		...
		*}
		...
		<p class="required select" style="display:none;visibility:hidden;">
		...
		<p class="required id_state select">
			<label for="id_state">{l s='Area'}<!--State--> <sup>*</sup></label>
		...
		<p class="text">
        {if isset($smarty.post.phone)}{$phone=$smarty.post.phone}{else}{if isset($address->phone)}{$phone=$address->phone}{/if}{/if}
        {$phonePrefix=substr($phone,0,1)}
        {$phoneSufix=substr($phone,1)}
			<label for="phone">{l s='Home phone'}</label>
			<select id="phone_prefix" name="phone_prefix">
            	 <option value="" >-</option>
                 <option value="4" {if $phonePrefix==4}selected="selected"{/if}>4</option>
            </select>
            <input type="text" id="phone" name="phone" value="{$phoneSufix}" />
		</p>
		<p class="text">
                {if isset($smarty.post.phone_mobile)}{$phone_mobile=$smarty.post.phone_mobile}{else}{if isset($address->phone_mobile)}{$phone_mobile=$address->phone_mobile}{/if}{/if}
        {$phone_mobilePrefix=substr($phone_mobile,0,2)}
        {$phone_mobileSufix=substr($phone_mobile,2)}
			<label for="phone_mobile">{l s='Mobile phone'}</label>
            	<select id="phone_mobile_prefix" name="phone_first">                
                 <option value="" >-</option>
                 <option value="50" {if $phone_mobilePrefix==50}selected="selected"{/if}>50</option>
                 <option value="52"{if $phone_mobilePrefix==52}selected="selected"{/if}>52</option>
                 <option value="55"{if $phone_mobilePrefix==55}selected="selected"{/if}>55</option>
                 <option value="56"{if $phone_mobilePrefix==56}selected="selected"{/if}>56</option>
                </select>
