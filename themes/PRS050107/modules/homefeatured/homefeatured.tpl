...
<div class="struct-left" style="height: 0px; width: 0px; opacity: 0; overflow: hidden;"></div>
					...
					<a href="{$link->getImageLink($product.link_rewrite, $product.id_image, 'thickbox_default')}" class="product_image product_img_link"><img src="{$link->getImageLink($product.link_rewrite, $product.id_image, 'home_default')}	" height="{$homeSize.height}" width="{$homeSize.width}" alt="{$product.name|escape:html:'UTF-8'}" />{if isset($product.new) && $product.new == 1}<span class="new">{l s='New' mod='homefeatured'}</span>{/if}</a>

					<h5><a title="{$product.name|truncate:50:'...'|escape:'htmlall':'UTF-8'}">{$product.name|truncate:35:'...'|escape:'htmlall':'UTF-8'}</a></h5>
